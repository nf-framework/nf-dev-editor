    ...
    select(opts) {
        ...
        return new Promise( r => {
            root.addEventListener('click', (e) => {
                ...
                let node = selector.findSelectable(e.composedPath(), this.root);
                try {
                    console.log('[nf-dev-editor][domselector][click]', {
                        type: this.type,
                        rawTarget: e.target?.localName || e.target?.nodeName || null,
                        selectedNode: node?.localName || node?.nodeName || null,
                        pathLength: e.composedPath?.().length || 0
                    });
                } catch (_err) {
