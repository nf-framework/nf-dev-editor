...
function normalizeSelectableTarget(target, root) {
    ...
    if (rootNode instanceof ShadowRoot && rootNode !== root) {
        ...
        const hostName = String(host?.localName || '').toLowerCase();
        const inPlatformComponentShadow = hostName.startsWith('pl-') && !hostName.startsWith('pl-form-');
        if (inPlatformComponentShadow) {
            // Клик по внутренней разметке pl-* компонента должен выбирать сам компонент.
            node = host || node;
        } else if (!isInsideRoot(node, root)) {
            // Для стороннего shadow-dom оставляем прежний fallback на host.
            node = host || node;
        }
    }
