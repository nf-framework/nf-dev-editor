...
export function findSelectable(path, root, checker) {
    return path.find((s) => {
        ...
        if (typeof checker === 'function') {
            ...
            if (checked === false || checked == null) return false;
...
domSelector.registerSelectorClass({
    ...
    onSelect: (target) => {
        ...
        try {
            console.log('[nf-dev-editor][polylib-selector][onSelect]', {
                rawTarget: target?.localName || target?.nodeName || null,
                selected: selected?.localName || selected?.nodeName || null,
                runtimePath,
                templatePath,
                finalPath: path,
                hasGridModelColumn: Boolean(model?.column?.node)
            });
        } catch (_err) {
...
function execChangePropertyCommand(cmd) {
    ...
    try {
        console.log('[nf-dev-editor][polylib-selector][change-property]', {
            path: cmd.path || null,
            sourcePath: cmd.sourcePath || null,
            property,
            value,
            propType: propType?.name || null,
            tplTarget: tplTarget?.localName || tplTarget?.nodeName || null,
            domTarget: domTarget?.localName || domTarget?.nodeName || null,
            tis: Array.isArray(tis) ? tis.length : 0
        });
    } catch (_err) {
        // ignore logging errors
    }
    ...
    for (let ti of tis ) {
        ...
        try {
            console.log('[nf-dev-editor][polylib-selector][change-property][ti]', {
                property,
                value,
                target: rawTarget?.localName || rawTarget?.nodeName || null,
                targetPath: Array.isArray(targetPath) ? targetPath.join('.') : null
            });
        } catch (_err) {
            // ignore logging errors
        }
    ...
    if (appliedCount === 0 && domTarget?.nodeType === Node.ELEMENT_NODE) {
        ...
        try {
            console.log('[nf-dev-editor][polylib-selector][change-property][fallback-dom-target]', {
                path: cmd.path || null,
                property,
                value,
                target: domTarget?.localName || domTarget?.nodeName || null
            });
        } catch (_err) {
            // ignore logging errors
        }
