...
function buildRecursive(array, node, parent) {
    ...
    info._pitem = parent;
    array.push(info)
    let childNodes = node.localName === 'template' ? node.content.childNodes : node.childNodes;
...
function execChangePropertyCommand(cmd) {
    ...
    for (let ti of tis ) {
        if (value.match?.(/({{.*}}|\[\[.*]])/) ){
            ...
            let domTarget = findByNPath({childNodes:ti._nodes}, path);
